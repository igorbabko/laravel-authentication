...
<body class="min-h-screen bg-gray-50">
    ...
    <header class="flex items-center justify-between p-6">
        ...
        <div class="flex gap-2">
            <a href="{{ route('register') }}" class="rounded-md bg-gray-200 py-2 px-4 font-semibold text-gray-900 shadow-lg transition duration-150 ease-in-out hover:bg-gray-300 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2">Register</a>
        </div>
    </header>
    <main class="flex flex-col justify-center p-6 pb-12">
        <div class="mx-auto max-w-md">
            ...
            <h2 class="mt-2 text-2xl font-bold text-gray-900 sm:mt-6 sm:text-3xl">Welcome back!</h2>
        </div>
        <div class="mx-auto mt-6 w-full max-w-md rounded-xl bg-white/80 p-6 shadow-xl backdrop-blur-xl sm:mt-10 sm:p-10">
            <div class="flex gap-3 rounded-md border border-red-500 bg-red-50 p-4">
            ...
            <form action="{{ route('login') }}" method="POST" autocomplete="off" novalidate class="space-y-6">
                @csrf
                <div>
                    ...
                    <div class="relative mt-1 rounded-md shadow-sm">
                        ...
                        <input type="email" id="email" name="email" value="{{ old('email') }}" class="{{ $errors->has('email') ? 'border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 text-gray-900 placeholder-gray-400 focus:border-green-500 focus:ring-green-500' }} w-full rounded-md pl-10 pr-10 text-sm" placeholder="john@example.com" />
                        @if ($errors->has('email'))
                ...
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('dashboard') }}" class="text-sm font-medium text-green-600 hover:text-green-500">Forgot your password?</a>
                </div>

                <div>
                    <a href="{{ route('dashboard') }}" class="flex items-center justify-center rounded-md bg-green-600 py-2 px-4 font-semibold text-white shadow-lg transition duration-150 ease-in-out hover:bg-green-700 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">Log In</a>
                </div>
            </form>
        </div>
    </main>
